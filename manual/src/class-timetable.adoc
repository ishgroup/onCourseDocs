...
image::images/class_type.png[title='This session has three tutors assigned, all of them with the same tutor roster as the session']

For a <<classes-aboutSelfPaced,self-paced>> class, ensure the 'self-paced' switch is turned on within this section, then save and close the record. For Hybrid classes, make sure the 'Is Hybrid' switch is turned on and that you set a minimum number of sessions for completion.
