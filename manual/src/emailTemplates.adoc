...
An event that sends a message is defined by a
link:scripts.html[script].
...
To understand the entities, objects and attributes available to use in building a template, we recommend you download Cayenne Modeler and read the section about link:#dataModel[using the onCourse data model].
...
link:scripts.hmtl[scripts], message templates may also be sent manually, or new templated created just to send manual messages.
