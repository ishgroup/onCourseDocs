...
This is a simple integration that uses a script to iterate through different course modules and creates a separate enrolment in TalentLMS for each unit in their TalentLMS system.

Enter your TalentLMS URL into 'Base url' and, your TalentLMS API key into 'API key'. Additionally, for this integration to work it's very important you keep your courses in onCourse, and your courses in TalentLMS in sync. The integration doesn't replicate courses from onCourse to TalentLMS, and will fail if the script cannot find a course in TalentLMS.

Your courses in TalentLMS must use the following naming convention - [courseCode][classCode][moduleNationalCode]. For example, if your Course Code is ENG101, the class code is 43, and the module code is BELLRN201, then your course in TalentLMS should be named ENG10143BELLRN201.

The integration will create a user in TalentLMS if they don't already exist. The details will be:

* login = email
* password = random password, 8 characters in length
* type = Learner-Type

On student enrolment, a new student to TalentLMS will receive a confirmation email from them.

==== LearnDash

image::images/learndash_integration.png[title='LearnDash Integration window']

This integration will push enrolments created in onCourse into LearnDash courses.

LearnDash is an LMS plugin for Wordpress websites. For this integration to work, you must have installed on your Wordpress site the 'Application Password' Wordpress plugin, then adjust the .htaccess config file (see https://github.com/WordPress/application-passwords/wiki/Basic-Authorization-Header----Missing for exact instructions).

Generate a password for any admin user though be aware that this is the user information you will enter in the integration window. Also enter your site URL in to the 'Base url' field and click Save.

You will need to keep your courses in onCourse and LearnDash in sync by ensuring that the course slug in LearnDash and the course code in onCourse are exactly the same. If onCourse can't find the course in LearnDash by matching its course code with the LearnDash course slug, the integration will do nothing.
