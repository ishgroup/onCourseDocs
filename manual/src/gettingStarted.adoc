...
Your favourites will always appear at the top of the global navigation, and it will _always_ containg Checkout (Quick Enrol).
...
==== On Demand Scripts (Admin only)

On Demands Scripts appear only for admin users, and can be triggered manually from the Automation category on the Dashboard. They will appear here when three conditions are met:
