...
. New classes appear with every box checked. To quickly uncheck them, hover your mouse over each outcome and click the chevron that appears, then select 'Not Assigned'. Once done for all outcomes, this will give you a fresh window.
. Just like when marking student attendance, click any circle to add a check mark. This will mark that outcome as being taught in that session, and will adjust that outcomes start and end dates within the class training plan accordingly. You can see these dates below each outcome heading as you add check marks.
. Every outcome should have at least one check mark next to it. A combination of these dates, the tutors for each session and other class details form the training plan.
