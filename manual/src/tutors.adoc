...
Access Tutor records by searching for 'Tutors' on the Dashboard. The Tutors window is just the Contacts window with the Tutors filter pre-selected. It operates exactly the same as the <<contacts, contacts>> window does.
...
image::images/tutor_record_general_tab.png[title='A tutors onCourse contact page']
...
For information on how to do this, read the <<generalPrefs-messaging, General Preferences documentation>>


. Open the tutor window and search for the tutor/s you're looking for using the search functions.
